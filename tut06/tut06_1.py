...
def attendance_report():
    ...
    list_class =["28/08","01/08","04/08","08/08","22/08","25/08","29/08","01/09","05/09","08/09","12/09"]
    dict_rollno ={}
    dict_time ={}
        ...
        if timestamp[i][0:10]==timestamp[x][0:10] and timestamp[i][0:5] in list_class:
            ...
            for j in range(1,n):
                if dict_rollno[rollNo[j]]>1:
                    ...
                    print(j)
                    timestamp_1.append(dict_time[rollNo[j]])
    ...
    for i in range(1,n):
        with open(rollNo[i]+".csv","w",newline ="") as file:
            writer = csv.writer(file)
            writer.writerow(["Roll","Name","total_lecture_taken","attendance_count_actual","attendance_count_fake","attendance_count_absent","Percentage (attendance_count_actual/total_lecture_taken) 2 digit decimal "])
            writer.writerow([rollNo[i],name[i],"11",dict_attendance_count_actual[rollNo[i]],dict_attendance_count_fake[rollNo[i]],11-dict_attendance_count_actual[rollNo[i]],np.round_((dict_attendance_count_actual[rollNo[i]]/11)*100,2)])
