...
last.write('6. S & A')
first.write('7. C & S & A')
last.write('8. all combine')
option2=0
tch = st.number_input('Chose Filtering Method From Above:',0,8)
if tch==1:
...
if option2==1:
    first,last = st.columns(2)
    first.write('1. previous point')
    last.write('2. 2*last-2nd_last')
    first.write('3. overall_mean')
    last.write('4. 12_point_strategy')
    first.write('5. mean of previous 2 point')
    last.write('6. all seqential')
    first.write('7. all parallel')
    sch = st.number_input('Chose Replacement Method From Above:',0,7)
    if st.button("click to begin"):
        start_time = datetime.now() 
        print('p1',start_time.strftime("%c"))
        ...
        with zipfile.ZipFile('Results_v2.zip', 'w') as zipObj:
